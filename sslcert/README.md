...
``` 
//on local
git clone https://github.com/zowe/clubes
cd clubes
npm install
npm start
```

Open your local browser and verify the clubes is working by accessing:     
...
cd clubes
npm run build
scp -r dist ibmuser@my.mainframe.com:</usr/lpp/extender>/clubes
```
...
cd </usr/lpp/extender>/clubes
npm install
```

## Method 2: From Artifactory
### 1) Download latest pax from artifactory
Get latest package from [artifactory](https://zowe.jfrog.io/artifactory/libs-snapshot-local/org/zowe/clubes/1.0.0-SNAPSHOT)
...
Please see static definition file `clubes.yml`      
It configures service endpoint as `clubes` with property `serviceId`     
...
`https://my.mainframe.com:7554/api/v1/clubes/accounts/`            
`https://my.mainframe.com:7554/api/v1/clubes/accounts/1/`            
`https://my.mainframe.com:7554/api/v1/clubes/accounts/1/cars/`              
